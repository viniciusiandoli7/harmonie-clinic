...
export function buildSafetyAlerts(patient: any): SafetyAlert[] {
  ...
  if (a.cancerHistory) alerts.push({ level: "WARNING", title: "Histórico de câncer", message: "Paciente informou histórico de câncer.", recommendation: "Avaliar liberação médica e contraindicações conforme procedimento planejado." });
  if (hasText(a.circulationProblems)) alerts.push({ level: "WARNING", title: "Trombose/circulação", message: a.circulationProblems, recommendation: "Avaliar risco vascular, histórico de trombose e necessidade de liberação antes de procedimentos injetáveis." });
