...
export default function MarketingStudioPage() {
  ...
  const [monthlyIndicators, setMonthlyIndicators] = useState({ seguidores: 0, mensagens: 0, agendamentos: 0, comparecimentos: 0, fechamentos: 0, depoimentos: 0, googleReviews: 0, indicacoes: 0 });

  // ESTADO DE CAMADAS UNIFICADO (Totalmente Arrastável)
  const [layers, setLayers] = useState<Layer[]>([
    { id: "logo", type: "logo", x: 50, y: 12, text: "MARIANA CARMONA", fontSize: 10 },
    { id: "headline", type: "text", x: 50, y: 45, text: "Beleza Natural", fontSize: 32, italic: true },
    { id: "subheadline", type: "text", x: 50, y: 55, text: "Rejuvenescimento natural e elegante.", fontSize: 13 },
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      link.download = `mariana-marketing-${Date.now()}.png`;
      link.href = dataUrl;
      link.click();
  ...
  return (
    <div className="flex h-screen bg-[#F0F0F0] overflow-hidden font-sans antialiased text-[#1E1A18]"
         onMouseMove={handleMouseMove}
         onMouseUp={() => setDraggingId(null)}>
      
      {/* 1. SIDEBAR DE CONTROLE (MARIANA STYLE) */}
      <aside className="w-[360px] bg-white border-r border-[#EEECE7] flex flex-col z-20 shadow-xl">
        <div className="p-8 border-b border-[#F7F2EA]">
          <p className="text-[10px] font-bold tracking-[0.4em] text-[#5A1F2B] uppercase italic">Creative Studio</p>
          <h1 className="text-3xl font-serif mt-1italic">Marketing</h1>
        </div>

        <div className="flex-1 overflow-y-auto p-8 space-y-10 scrollbar-thin">
          <section className="rounded-3xl border border-[rgba(90,31,43,.12)] bg-[#F7F2EA]/70 p-5">
            <HeaderField icon={<Focus size={14} className="text-[#5A1F2B]" />} label="Indicadores Mensais" />
            <div className="grid grid-cols-2 gap-3">
              {Object.entries(monthlyIndicators).map(([key, value]) => (
                <label key={key} className="block">
                  <span className="mb-1 block text-[8px] font-bold uppercase tracking-[0.16em] text-[#5B3A2E]/55">{key.replace(/([A-Z])/g, " $1")}</span>
                  <input
                    type="number"
                    value={value}
                    onChange={(e) => setMonthlyIndicators((prev) => ({ ...prev, [key]: Number(e.target.value) }))}
                    className="h-10 w-full border px-3 text-[12px]"
                  />
                </label>
              ))}
            </div>
            <div className="mt-4 h-2 overflow-hidden rounded-full bg-[#D8C4AE]/45">
              <div className="h-full rounded-full bg-[#5A1F2B]" style={{ width: `${Math.min(100, monthlyIndicators.fechamentos * 10)}%` }} />
            </div>
          </section>

          {/* Gerador de Imagem */}
          <section>
            <HeaderField icon={<Sparkles size={14} className="text-[#5A1F2B]" />} label="AI Visual Engine" />
            <textarea 
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              placeholder="Descreva a estética da imagem... (Ex: Luxo minimalista, luz dourada, tons pastéis)"
              className="w-full p-4 bg-[#F7F2EA] border border-[#E9DEC9] text-[12px] outline-none min-h-[110px] focus:border-[#5A1F2B] transition-colors resize-none italic"
            />
            <button className="w-full mt-4 bg-[#1E1A18] text-white py-4 text-[11px] font-bold uppercase tracking-[0.2em] hover:bg-[#5A1F2B] transition-all flex items-center justify-center gap-2 group shadow-md">
          ...
          {selectedLayer && (
             <section className="pt-8 border-t border-[#F7F2EA] space-y-6 animate-in fade-in slide-in-from-top-4 duration-300">
                  ...
                  <div className="space-y-4">
                     <div className="space-y-2">
                        ...
                        <div className="flex items-center gap-4">
                           <input type="range" min="50" max="250" value={selectedLayer.scale} onChange={(e) => updateLayer(selectedLayer.id, { scale: Number(e.target.value) })} className="w-full accent-[#5A1F2B] h-1" />
                           <span className="text-xs font-mono text-[#5A1F2B]">{selectedLayer.scale}%</span>
                        </div>
                     </div>
                ...
                {selectedLayer.text !== undefined && (
                   <div className="space-y-1">
                      ...
                      <input type="text" value={selectedLayer.text} onChange={(e) => updateLayer(selectedLayer.id, { text: e.target.value })} className="w-full border-b border-[#EEE] py-2 text-sm outline-none focus:border-[#5A1F2B]" />
                   </div>
                )}
             </section>
          )}
        ...
        <div className="p-8 border-t border-[#F7F2EA]">
          <button onClick={handleExport} className="w-full bg-[#5A1F2B] text-white py-4 text-[11px] font-bold uppercase tracking-[0.2em] flex items-center justify-center gap-2 hover:brightness-110 transition-all shadow-lg">
      ...
      <main className="flex-1 relative flex flex-col items-center justify-center p-12 bg-[#EAEAEA] bg-[radial-gradient(#5A1F2B_0.5px,transparent_0.5px)] [background-size:24px_24px] [background-opacity:0.05]"
        ...
        >
          ...
          {layers.map((layer) => {
            ...
            if (layer.type === 'image') {
              return (
                <div key={layer.id}
                     className={`absolute cursor-move overflow-hidden transition-all duration-300 flex items-center justify-center ${isSelected ? 'ring-2 ring-[#5A1F2B] z-20 shadow-2xl' : 'hover:ring-1 hover:ring-white/10 z-0'}`}
                     style={{ 
            ...
            return (
              <div key={layer.id}
                   className={`absolute cursor-move select-none p-3 group transition-all duration-300 ${isSelected ? 'ring-1 ring-[#5A1F2B] bg-black/20' : 'hover:ring-1 hover:ring-white/10'}`}
                   style={{ 
                     left: `${layer.x}%`, top: `${layer.y}%`, 
                     transform: 'translate(-50%, -50%)',
                ...
                <div 
                  ...
                  style={{
                    ...
                    color: layer.type === 'cta' ? '#5A1F2B' : 'white',
                    ...
                    border: layer.type === 'cta' ? '1px solid #5A1F2B' : 'none',
...
function FormatBtn({ active, label, onClick }: any) {
  return (
    <button onClick={onClick} className={`py-3 text-[10px] font-black uppercase tracking-widest border transition-all rounded-sm ${active ? 'bg-[#1E1A18] text-white border-[#1E1A18]' : 'bg-white text-[#96A4C1] border-[#EEE] hover:border-[#5A1F2B]'}`}>
      {label}
    </button>
  );
}
...
function SelectionHandles() {
  const handleStyle = "absolute w-2 h-2 bg-[#5A1F2B] z-30";
  return (
    <>
